...
import { ModuleDiary } from '@/components/dashboard/module-diary';

function DiaryContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const from = searchParams.get('from');
  const initialEntryId = searchParams.get('id') ? Number(searchParams.get('id')) : null;
  const handleBack = () => {
    if (from) {
  ...
  return (
    <div className="h-[100dvh] overflow-y-auto flex flex-col">
      ...
      <div className="flex items-center gap-2 px-4 pt-14 pb-2 flex-shrink-0">
        ...
        <h1 className="text-base font-medium text-gray-700">日记回音</h1>
      </div>

      {/* 模块内容 */}
      <div className="flex-1 min-h-0 px-3 pb-4">
        <ModuleDiary initialEntryId={initialEntryId} />
      </div>
    </div>
  );
}
